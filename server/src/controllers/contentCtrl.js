...
const removeProduct = async (req, res) => {
	const prodId = req.params.id;
	const deleteProd = await userMongoContainer.deleteCartProd(prodId, req.user.id);
	res.send(deleteProd);
};
...
const buyCart = async (req, res) => {
	...
	const { cart, email, username, phone } = await userMongoContainer.getOneDoc(userId);
    ...
    <tr><td><p>Total: ${cart.reduce((acc, current) => acc + current.price, 0)}</p></td></tr>
  </table>
	    `;
...
export { getProducts, getUserData, getCart, addProductsToCart, removeProduct, buyCart, emptyCart };
